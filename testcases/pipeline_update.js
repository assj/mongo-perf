 ...
 */
tests.push({
    name: 'Update.Pipeline.IncWithIndex',
    tags: ['update', 'core', 'indexed', 'pipeline-update'],
 ...
 */
tests.push({
    name: 'Update.Pipeline.IncWithIndexUpsert',
    tags: ['update', 'core', 'indexed', 'pipeline-update'],
    pre: function(collection) {
        collection.drop();
        collection.ensureIndex({x: 1});
 ...
 */
tests.push({
    name: 'Update.Pipeline.IncFewLargeDocLongFields',
    tags: ['update', 'regression', 'pipeline-update'],
    pre: function(collection) {
        collection.drop();
 ...
 */
tests.push({
    name: "Update.Pipelines.SetWithMultiIndex.String",
    tags: ['update', 'indexed', 'regression', 'pipeline-updates'],
    pre: function(collection) {
        collection.drop();
        var docs = [];
        for (var i = 0; i < 4800; i++) {
...
tests.push({
    ...
    tags: ['update', 'mms', 'core', 'single_threaded', 'pipeline-updates'],
    pre: function(collection) {
        collection.drop();
...
tests.push({
    ...
    tags: ['update', 'indexed', 'pipeline-updates'],
    pre: function(collection) {
        collection.drop();
        var docs = [];
        for (var i = 0; i < 4800; i++) {
...
tests.push({
    ...
    pre: function(collection) {
        ...
        var docs = [];
        for (var i = 0; i < 2400; i++) {
            docs.push({_id: i, x: NumberDecimal(i), y: NumberLong(i), oldYs: [NumberLong(i - 12)]});
        }
        collection.insert(docs);
    },
    ops:
